...
namespace Assets.scripts.GUI
{
    public class DrawObjectiveList : MonoBehaviour
    {
        ...
        public Text BtnText;
        private float _timer = 8f, _continueTimer = 3f;
        ...
        private void Start()
        {
            ...
            //_height = Screen.height * 0.12f;
            //_width = Screen.width * 0.17f;
        ...
        public void MinimizeInterface(bool min)
        {
            ...
            {
                //if(Application.loadedLevelName == "City" && !FirstTimePlay)
        ...
        private void InterfaceStates()
        {
            ...
            if (Application.loadedLevel == 2)
            {
                if (Minimized)
                {
                    ShowMiniMap = !(PerformanceProcessor.CurrentProfileModel.Difficulty > 9.5f);
                    this.GetComponent<Canvas>().sortingOrder = 3;
                    _miniMap.gameObject.SetActive(ShowMiniMap);

                    if ((_gm.IsCalibrationRequired && CalibrationGUI.CalibDone) || !_gm.IsCalibrationRequired)
                    {
                        MiniMapSmall();
                        _bg.SetActive(false);
                    }
                ...
                else
                {
                    ...
                    if ((_gm.IsCalibrationRequired && CalibrationGUI.CalibDone) || (!_gm.IsCalibrationRequired && LevelSet))
                    {
                        MiniMapBig();
                        _bg.SetActive(false);
                    }
        ...
        private void DrawItemsList()
        {
            ...
            if (DrawingList && Application.loadedLevelName != "GameOver")
            {
                ...
                var containerRectTransform = _verticalLayout.rectTransform;
                ...
                if (newItem != null)
                {
                    ...
                    //var tick = newItem.GetComponentInChildren<Image>();
                
                    itemText.text = _description;

                    //tick.gameObject.SetActive(_objManager.GetCurrentObjective != null && _objManager.GetCurrentObjective.Completed);
                }

                //var scrollHeight = 2 * _height;
            
                //containerRectTransform.sizeDelta = new Vector2(_width, scrollHeight);
                //containerRectTransform.anchoredPosition = new Vector2(_width, containerRectTransform.anchoredPosition.y);
