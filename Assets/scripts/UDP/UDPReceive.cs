...
namespace Assets.scripts.UDP
{
    public class UdpReceive : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (_gm.IsCalibrationRequired)
            {
                ...
                */
                ...
                if (Input.GetKeyDown(KeyCode.KeypadEnter) && !calibrate)
                {
        ...
        private void ChoseDevice(string selected)
        {
            ...
            if (System.Threading.Thread.CurrentThread.CurrentCulture.TextInfo.ToTitleCase(selected.ToUpper()).Contains("TOBII"))//CENTERGAZEPOINT
            {
                ClearLists();
                //clear lists before using other _joint for _tracking		
                //fix to point into a _joint;
                _tracking = false;
                _eyetrack = true;
