...
namespace Assets.scripts.Settings
{
    public class Language : MonoBehaviour {
        ...
        public static string vSpatial, naming, abstraction, delRecall, orient, execFunc, idiom, edu, moca, models, cars;
...
*/
            ...
            if (File.Exists(filepath))
            {
                ...
                XmlNodeList navUi = xmlDoc.GetElementsByTagName("Navigation");
                ...
                foreach (XmlNode wordsInfo in navUi)
                {
                    XmlNodeList xmlcontent = wordsInfo.ChildNodes;
                    ...
                    {
                        if (words.Name == "Speed")
                            Speed = words.InnerText;
                        if (words.Name == "Exp")
                            Exp = words.InnerText;
                        if (words.Name == "Log")
                            Log = words.InnerText;
                        if (words.Name == "Prop")
                            Prop = words.InnerText;
                        if (words.Name == "MoveAndRot")
                            MoveAndRot = words.InnerText;
                        if (words.Name == "Sensitivity")
                            Sensitivity = words.InnerText;
                        if (words.Name == "TopThresh")
                            TopThresh = words.InnerText;
                        if (words.Name == "BottomThresh")
                            BottomThresh = words.InnerText;
                        if (words.Name == "SidesThresh")
                            SidesThresh = words.InnerText;
                        if (words.Name == "Cursor")
                            Cursor = words.InnerText;
                        if (words.Name == "Teleport")
                            Teleport = words.InnerText;
                        if (words.Name == "NodeTime")
                            NodeTime = words.InnerText;
                    }
                }
            }
        }//load from xml
