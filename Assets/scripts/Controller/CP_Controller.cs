...
namespace Assets.scripts.Controller
{
    public class CP_Controller : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            _tracking = GameObject.Find("TrackingPanel");
        ...
        private void Update()
        {
            ...
            if (Selected && SelectedObject != null && SelectedObject.GetComponent<BoxCollider>().enabled == false)
                SelectedObject.transform.FindChild("Cylinder").renderer.material.color = _deSelectedColor;

            if (!DrawObjectiveList.Minimized && !Tracking.IsTracking)
            {
            ...
            if (cl.tag == "item_1" || cl.tag == "directionalarrow" || cl.tag == "SelectionArrows" || cl.tag == "keyNumber" || cl.tag == "continueButton")
            {
                ...
                switch (cl.tag)
                {
                    case "item_1":
                        ...
                        SelectedObject.transform.FindChild("Cylinder").renderer.material.color = _selectedColor;
                        break;
