...
namespace Assets.scripts.Locations
{
    public class EnterRoom : MonoBehaviour {
        ...
        private void Update()
        {
            if (Application.loadedLevelName == "City" && (Controller.Controller.B6() || NavigationTimer.JumpToDestination))
            {
                switch (_objmanager.GetCurrentObjective.name)
                {
                    case "SuperMarket":
                        _player.transform.position = new Vector3(-2.78f, 2f, 28.51f);
                        _player.transform.rotation = Quaternion.Euler(new Vector3(0, 270, 0));
                        break;
                    case "PostOffice":
                        _player.transform.position = new Vector3(145.5f, 2f, 100f);
                        _player.transform.rotation = Quaternion.Euler(new Vector3(0, 90f, 0));
                        break;
                    case "Pharmacy":
                        _player.transform.position = new Vector3(81f, 2f, -91.9f);
                        _player.transform.rotation = Quaternion.Euler(new Vector3(0, 180, 0));
                        break;
                    case "Bank":
                        _player.transform.position = new Vector3(-50f, 2f, -76.64f);
                        _player.transform.rotation = Quaternion.Euler(new Vector3(0, 270f, 0));
                        break;
                    case "Home":
                        _player.transform.position = new Vector3(2.175f, 2f, 101.21f);
                        _player.transform.rotation = Quaternion.Euler(new Vector3(0, 90f, 0));
                        break;
                    case "FashionStore":
                        _player.transform.position = new Vector3(-29.5f, 2f, -2.4f);
                        _player.transform.rotation = Quaternion.Euler(new Vector3(0, 180, 0));
                        break;
                    case "Kiosk":
                        _player.transform.position = new Vector3(-8f, 2f, -135f);
                        _player.transform.rotation = Quaternion.Euler(new Vector3(0, 180, 0));
                        break;
                    case "Park":
                        _player.transform.position = new Vector3(98f, 2f, 45f);
                        _player.transform.rotation = Quaternion.Euler(new Vector3(0, 90f, 0));
                        break;
                    default:
                        break;
                }
                NavigationTimer.JumpToDestination = false;
            }

            if (_objmanager.GetCurrentObjective == null) return;
            
            if (_objmanager.GetCurrentObjective.name == LevelToLoad)
                ObjRange.SetActive(true);
            else
                ObjRange.SetActive(false);
            ...
            if (Vector3.Distance(this.gameObject.transform.position, _player.transform.position) < 12.0f)
                _objectiveRequiredToEnter = this.gameObject.name;

            _canDisplay = Vector3.Distance(gameObject.transform.position, _player.transform.position) < 12.0f ? true : false;
                ...
                _objmanager.GetCurrentObjective.name == _objectiveRequiredToEnter)
            {
                ...
                if (Controller.Controller.B1())
                {
                    ...
                    {
                        case "SuperMarket":
                            GameManager.PlayerData.Playerposition = new Vector3(this.transform.position.x + 1, 3,
                                this.transform.position.z + 13.6f);
                        ...
                        case "Pharmacy":
                            ...
                            break;
                        case "Bank":
                            GameManager.PlayerData.Playerposition = new Vector3(this.transform.position.x -3f, 3,
                                this.transform.position.z - 10f);
                            GameManager.PlayerData.Playerrotation = new Vector3(this.transform.localEulerAngles.x,
                                90, this.transform.localEulerAngles.z);
                            break;
                        case "Home":
                            GameManager.PlayerData.Playerposition = new Vector3(this.transform.position.x -0.3f, 3,
                                this.transform.position.z - 10.1f);
                            GameManager.PlayerData.Playerrotation = new Vector3(this.transform.localEulerAngles.x,
                                -90, this.transform.localEulerAngles.z);
                            break;
                        case "FashionStore":
                            GameManager.PlayerData.Playerposition = new Vector3(this.transform.position.x - 13, 3,
