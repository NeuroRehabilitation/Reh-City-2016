...
namespace Assets.scripts.RehaTask.RehaTask_GameLogic
{
    public class LoadGame : MonoBehaviour
    {
        ...
        public static bool CanSetRehaTask = true;
        ...
        public static List<int> SessionHome = new List<int>();
        public static List<string> SessionSubCategory = new List<string>();
                ...
                for (var i = 0; i < SpawnTiles.TotCategories; i++)
                {
                    ...
                    */
                    ...
                    do
                    {
                        ...
                        var subCategory = tempList[rand].Substring(0, 2);
                        
                        Debug.Log(SpawnTiles.Cats[i] + subCategory);

                        if (!SessionSubCategory.Contains(SpawnTiles.Cats[i] + subCategory))
                        {
                            subCat = subCategory;
...
*/
            ...
            for (var i = 0; i < SpawnTiles.Cats.Count; i++)
            {
                if (SpawnTiles.Cats[i] == SpawnTiles.Folder)
                {
                    ...
                    do
                    {
                        var rand = UnityEngine.Random.Range(0, SpawnTiles.ImagesNames[_catArray].Count);
                        var subCategory = SpawnTiles.ImagesNames[_catArray][rand].Substring(0, 2);

                        if (!SessionSubCategory.Contains(SpawnTiles.Folder + subCategory))
                        {
                            SubCategory = subCategory;
                        }
                    } while (SubCategory == "");

                    SessionSubCategory.Add(SpawnTiles.Folder + SubCategory);
                    //Debug.Log(SpawnTiles.Folder + SubCategory + SessionSubCategory.Count);
                        ...
                        foreach (var fi3 in smFiles3)
                        {
                            if (fi3.Extension == ".jpg" || fi3.Extension == ".png" || fi3.Extension == ".JPG" || fi3.Extension == ".PNG")
                            {
                                ...
                                */
                                ...
                                //Debug.Log(catImagesToLoad);
                                //Debug.Log(SubCategory);
                                //for (var a = 0; a < catImagesToLoad; a++)
                                //{
                                    
                                    if (/*fi3.Name == SpawnTiles.ImagesNames[i][a] && */fi3.Name.StartsWith(SubCategory) && LevelCategory.Count< catImagesToLoad)
                                    {
                                        //Debug.Log(fi3.Name);
