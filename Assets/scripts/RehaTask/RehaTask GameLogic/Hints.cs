...
namespace Assets.scripts.RehaTask.RehaTask_GameLogic
{
    public class Hints : MonoBehaviour {
        ...
        private void OrderHints()
        {
            if(!Ordered && SpawnTiles.RenderPointer && SpawnTiles.Sequence)
            {
                for(var i = 0; i < SpawnTiles.SelectionList.Count; i++)
                {
                    ...
                    {
                        if(SpawnTiles.Hints[b].transform.Find("ImageDisplay").renderer.material.mainTexture == SpawnTiles.SelectionList[i])
                        {
        ...
        private void ShowHint()
        {
            if (SpawnTiles.Hints.Count > 0)
            {
                foreach (var t in SpawnTiles.Incorrect)
                {
                    ...
                    _incorrectTile.gameObject.collider.enabled = false;
                }
                ...
                {
                    if (CollisionDetection.OpenCards.Count > 0 && CollisionDetection.OpenCards.Count < SpawnTiles.Pairs && CanDisplayHint )
                    {
                        for (var i = 0; i < SpawnTiles.Hints.Count; i++)
                        {
                            for (var b = 0; b < CollisionDetection.OpenCards.Count; b++)
                            {
                                if (SpawnTiles.Hints[i].transform.Find("ImageDisplay").renderer.material.mainTexture ==
                                    CollisionDetection.OpenCards[b].transform.Find("ImageDisplay").renderer.material.mainTexture)
                                {
                                    _hintToDisplay = i;
                                }
                            }
                        ...
                        _hintTile.transform.FindChild("frame").gameObject.tag = "Hint";
        ...
        private void AnimateCube()
        {
            ...
            {
                if ((SpawnTiles.Pairs > 0 && child.tag == "Hint") || SpawnTiles.Pairs == 0)
                {
                    child.gameObject.SetActive(true);
                    //cube color animation to point out where correct answers are
                    BlinkTimer -= Time.deltaTime;
                    var lerp = Mathf.PingPong(Time.time, 0.6f) / 0.6f;
                    _imageDisplay.gameObject.renderer.material.color = Color.Lerp(orange, Color.white, lerp);
                ...
                if (BlinkTimer < 0)
                {
                    ...
                    _imageDisplay.gameObject.renderer.material.color = Color.white;
                }
            }    
        }
    }
